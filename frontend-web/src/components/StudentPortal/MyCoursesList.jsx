...
export default function MyCoursesList({ courses = [] }) {
  return (
    <div className="glass rounded-2xl p-5">
      <h3 className="font-semibold text-white mb-4 flex items-center gap-2"><BookOpen size={16} className="text-[#667D9D]" /> My Courses</h3>
        ...
        {courses.length === 0 ? <div className="text-sm text-slate-600 py-8 text-center">No courses available.</div> : courses.map((course) => (
          <div key={course.courseId} className="rounded-xl bg-navy-900/50 border border-slate-800/60 p-3">
            ...
            <div className="mt-2 h-1.5 rounded-full bg-slate-800 overflow-hidden">
              <div className="h-full rounded-full bg-gradient-to-r from-[#16254F] to-[#667D9D]" style={{ width: `${Math.min(100, Math.round(((course.attendedSessions ?? 0) / Math.max(1, course.totalSessions ?? 1)) * 100))}%` }} />
            </div>
          </div>
        ))}
      </div>
