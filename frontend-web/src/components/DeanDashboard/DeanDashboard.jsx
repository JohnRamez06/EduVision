...
export default function DeanDashboard() {
	...
	return (
		<div className="min-h-screen bg-navy-950 text-slate-100 relative overflow-hidden">
			...
			<div className="relative z-10 max-w-7xl mx-auto px-5 md:px-7 py-6 md:py-8 space-y-6">
					...
					<div className="relative grid gap-6 lg:grid-cols-[1.3fr_0.9fr] items-center">
						<div>
							...
							<div className="flex flex-wrap gap-3 mt-5">
								...
								<span className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-slate-900/70 border border-slate-800 text-xs text-slate-400">
						...
						<div className="grid grid-cols-2 gap-3">
							<div className="rounded-2xl p-4 bg-slate-950/60 border border-slate-800">
								<p className="text-xs text-slate-500 mb-1">Average Engagement</p>
								<p className="text-3xl font-black text-white">{formatPct(summary.avgEngagement)}</p>
							...
							<div className="rounded-2xl p-4 bg-slate-950/60 border border-slate-800">
								...
								<p className="text-3xl font-black text-white">{formatPct(summary.avgAttendance)}</p>
							...
							<div className="rounded-2xl p-4 bg-slate-950/60 border border-slate-800">
								...
								<p className="text-3xl font-black text-white">{summary.totalCourses ?? courseEngagement.length ?? 0}</p>
							...
							<div className="rounded-2xl p-4 bg-slate-950/60 border border-slate-800">
								...
								<p className="text-3xl font-black text-white">{summary.activeSessions ?? 0}</p>
				...
				<div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
					...
					<WeeklyReportViewer reports={recentReports} />
				</div>
				...
				<div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
					...
					<div className="glass rounded-2xl p-5">
						<h2 className="font-semibold text-white mb-4">Backend Connection</h2>
						<div className="space-y-3 text-sm text-slate-400 leading-relaxed">
							<p>This page first loads `/facade/dean/dashboard` and falls back to composed admin, lecturer, and student data if the dean endpoint is not present.</p>
							<p>The Analytics-R service is exposed through the `VITE_SHINY_BASE_URL` link in the hero section for deeper reporting.</p>
						</div>
					</div>
				</div>
			</div>
