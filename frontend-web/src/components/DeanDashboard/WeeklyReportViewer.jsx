					...
					<table className="w-full text-sm">
						...
						<tbody className="divide-y divide-slate-800/60">
							{items.slice(0, 6).map((report, index) => {
								...
								return (
									<tr key={report.reportId ?? report.id ?? index} className="hover:bg-slate-800/20 transition-colors">
										<td className="py-3 pr-4 text-slate-200 font-medium">{report.title ?? report.name ?? `Report ${index + 1}`}</td>
										<td className="py-3 pr-4 text-slate-400 text-xs">{report.lecturerName ?? report.owner ?? '—'}</td>
										<td className="py-3 pr-4 text-slate-400 text-xs whitespace-nowrap">{formatDate(report.generatedAt ?? report.date ?? report.createdAt)}</td>
