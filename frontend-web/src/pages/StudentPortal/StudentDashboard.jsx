...
const AttBadge = ({ status }) => {
  ...
  const cls = s === 'present' ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20'
            : s === 'late'    ? 'text-amber-400 bg-amber-500/10 border-amber-500/20'
            : s === 'absent'  ? 'text-rose-400 bg-rose-500/10 border-rose-500/20'
            :                   'text-slate-400 bg-slate-700/40 border-slate-700/50'
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize border ${cls}`}>
      {status ?? '—'}
    </span>
  )
}
  ...
  return (
    <StudentLayout>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <StatCard
            icon={Brain} color="violet" label="Avg Concentration"
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-[11px] text-slate-500 uppercase tracking-wide">
                    ...
                    <th className="text-center pb-3 font-semibold pr-3">Concentration</th>
                    <th className="text-center pb-3 font-semibold pr-3">Attendance</th>
                    <th className="text-left pb-3 font-semibold">Emotion</th>
                  </tr>
                </thead>
                <tbody>
                  {data?.recentSummaries?.map(s => (
                    <tr key={s.sessionId} className="border-t border-slate-800/40 hover:bg-slate-800/30 transition-colors group">
