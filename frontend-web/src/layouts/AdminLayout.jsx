...
import {
  Shield, LayoutDashboard, Users, ShieldCheck, LogOut, Bell, Menu, Camera, ChevronRight,
} from 'lucide-react'
...
const PAGE_TITLE = {
  ...
  '/admin/face-enrollment': 'Face Enrollment',
}
...
export default function AdminLayout({ children }) {
  ...
  const pageTitle = PAGE_TITLE[location.pathname] ?? 'Admin Portal'

  const Sidebar = ({ mobile = false }) => (
    <aside className={`sidebar flex flex-col h-full ${mobile ? 'w-full' : 'w-60'}`}>
      {/* Logo */}
      <div className="flex items-center gap-3 px-5 py-[18px] border-b border-slate-200/80 dark:border-slate-800/50">
        ...
        <div>
          ...
          <span className="text-[10px] font-medium tracking-wide uppercase mt-0.5 block text-violet-600/80 dark:text-violet-400/80">Admin</span>
        </div>
      </div>
      ...
      <nav className="flex-1 px-2.5 py-4 space-y-0.5 overflow-y-auto">
        {NAV.map(({ to, icon: Icon, label, end }) => (
          ...
          >
            {({ isActive }) => (
              <>
                <span className={`shrink-0 transition-colors duration-150 ${isActive ? 'text-violet-600 dark:text-violet-400' : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300'}`}>
                  <Icon size={16} />
                </span>
      ...
      <div className="px-2.5 pt-3 pb-4 border-t border-slate-200/80 dark:border-slate-800/50 space-y-1">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800/30">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-xs font-semibold text-slate-800 dark:text-slate-200 truncate leading-tight">{user?.fullName ?? 'Admin'}</p>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <ThemeToggle />
            <button
              className="w-8 h-8 rounded-lg flex items-center justify-center transition-all cursor-pointer text-slate-400 hover:text-slate-700 dark:hover:text-white hover:bg-slate-200/70 dark:hover:bg-slate-800/70"
