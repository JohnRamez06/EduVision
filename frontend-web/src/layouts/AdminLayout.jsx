    ...
    <aside className={`sidebar flex flex-col h-full ${mobile ? 'w-full' : 'w-60'}`}>
      ...
      <div className="flex items-center gap-3 px-5 py-[18px] border-b border-slate-200/80 dark:border-slate-800/50">
        ...
        <div>
          ...
          <span className="text-[10px] font-medium tracking-wide uppercase mt-0.5 block text-[#667D9D]/80 dark:text-[#667D9D]/80">Admin</span>
        </div>
      </div>
      ...
      <nav className="flex-1 px-2.5 py-4 space-y-0.5 overflow-y-auto">
        {NAV.map(({ to, icon: Icon, label, end }) => (
          ...
          >
            {({ isActive }) => (
              <>
                <span className={`shrink-0 transition-colors duration-150 ${isActive ? 'text-[#667D9D] dark:text-[#667D9D]' : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300'}`}>
                  <Icon size={16} />
                </span>
