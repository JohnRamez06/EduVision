...
import {
  ...
  Lightbulb, ShieldCheck, User, LogOut, Bell, Menu, X,
} from 'lucide-react'
  ...
  const Sidebar = ({ mobile = false }) => (
    <aside className={`flex flex-col h-full bg-navy-900 border-r border-slate-800/60 ${mobile ? 'w-full' : 'w-60'}`}>
      ...
      <div className="flex items-center gap-2.5 px-5 py-5 border-b border-slate-800/60">
        ...
        <span className="font-bold text-white tracking-tight">EduVision</span>
      </div>
      ...
      <div className="px-3 py-4 border-t border-slate-800/60">
        <div className="flex items-center gap-3 px-3 py-2 rounded-xl mb-1">
          ...
          <div className="min-w-0">
            <p className="text-sm font-medium text-slate-200 truncate">{user?.fullName ?? 'Student'}</p>
