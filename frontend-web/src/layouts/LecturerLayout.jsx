    ...
    <aside className={`sidebar flex flex-col h-full border-r transition-colors duration-200 border-slate-200/80 dark:border-slate-800/50 ${mobile ? 'w-full' : 'w-60'}`}>
      ...
      <div className="flex items-center gap-3 px-5 py-[18px] border-b border-slate-200/80 dark:border-slate-800/50">
        ...
        <div>
          ...
          <span className="text-[10px] font-medium tracking-wide uppercase mt-0.5 block text-[#667D9D]/80 dark:text-[#667D9D]/80">Lecturer</span>
        </div>
      </div>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 cursor-pointer ${
                isActive
                  ? 'bg-[#ECECEC] dark:bg-[#667D9D]/20 text-[#060817] dark:text-[#ECECEC] border border-[#ACBBC6]/90 dark:border-[#667D9D]/40 shadow-sm'
            ...
            {({ isActive }) => (
              <>
                <span className={`shrink-0 transition-colors duration-150 ${
                  isActive
                    ? 'text-[#16254F] dark:text-[#ACBBC6]'
                    : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300'
                }`}>
                  <Icon size={16} />
                </span>
                <span className="flex-1">{label}</span>
                {isActive && <ChevronRight size={12} className="text-[#667D9D]/80 dark:text-[#ACBBC6]/80 shrink-0" />}
              </>
            )}
          </NavLink>
        ))}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        >
          ...
          <div className="flex items-center gap-1">
            <ThemeToggle />
            <AlertBell />
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#16254F] to-[#667D9D] flex items-center justify-center text-[11px] font-bold text-white ring-2 ring-transparent hover:ring-[#667D9D]/30 transition-all cursor-pointer ml-1">
              {initials}
            </div>
          </div>
        </header>
